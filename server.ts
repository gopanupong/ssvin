...
• **หัวข้อ Battery (แบตเตอรี่ - น้ำกลั่นและสภาพขั้วแบต):**
  - วิเคราะห์ระดับน้ำกลั่น (Electrolyte Level) ของแบตเตอรี่แต่ละลูกที่เห็นในภาพถ่ายอย่างละเอียด โดยสังเกตและระบุหมายเลขลูกแบตเตอรี่ (เช่น ลูกที่ 4, 5, 6, 24, 25, 26, 27, 28) จากสติกเกอร์ที่ติดอยู่ชัดเจนบนตัวถังแบตเตอรี่
  - เปรียบเทียบระดับน้ำเหลว/คราบน้ำกับเส้นขีดบนตัวถัง ได้แก่ เส้นระดับสูงสุด (MAX / Upper limit) และระดับต่ำสุด (MIN / Lower limit)
  - **100 คะแนน:** ระดับน้ำกลั่นของทุกลูกที่มองเห็นอยู่ระหว่างขีด UPPER (MAX) และ LOWER (MIN) อย่างสมเหตุสมผลและชัดเจน ขั้วแบตสะอาดไม่มีคราบเกลือ/คราบซัลเฟตสีขาวหรือขาวฟ้า
  - **50 คะแนน:** มีบางลูกที่ระดับน้ำกลั่นเริ่มอยู่ต่ำกว่ากึ่งกลางลงไปหาขีดล่าง หรือระดับน้ำค่อนข้างต่ำแต่ยังไม่หลุดขีดล่าง หรือภาพถ่ายเบลอ มืด หรือถ่ายระยะไกลจนระบุระดับน้ำกลั่นของแต่ละลูกได้ยากหรือไม่ชัดเจน
  - **0 คะแนน:** มีลูกใดลูกหนึ่งที่ระดับน้ำกลั่นแห้งต่ำกว่าขีดจำกัดล่าง (MIN/Lower Limit) หรือพบลักษณะความเสียหายบกพร่องที่ตัวโครงถัง (เช่น บวม ร้าว แตก รั่วซึม หรือมีคราบซัลเฟตหนามากที่เกาะขั้วแบต)
...
- หากวิเคราะห์ได้ว่าเป็น 'Battery' ต้องเขียนรายงานระบุให้ชัดเจนถึงระดับน้ำกลั่นของหมายเลขแบตเตอรี่แต่ละตัวที่ระบุได้ เช่น "จากการตรวจสอบระบบแบตเตอรี่ OPzS หมายเลข 4, 5, 6 พบว่าระดับน้ำกลั่นของทุกลูกอยู่กึ่งกลางปกติเหนือขีด MIN และอยู่ในระดับปลอดภัยระหว่างขีด Upper และ Lower, สภาพขั้วต่อสะอาดไม่มีเกลือซัลเฟต" (ระบุข้อมูลระดับน้ำกลั่นและสภาพขั้วของแบตเตอรี่แต่ละลูกในรูปภาพเป็นข้อความที่ชัดเจนที่สุด)
...
ตอบกลับในรูปแบบ JSON เท่านั้น โดยมีโครงสร้างดังนี้:
{
  ...
  "summary": "สรุปรายงานผลการวิเคราะห์อย่างละเอียดเป็นภาษาไทย พร้อมระบุข้อมูลเกี่ยวกับแบตเตอรี่แต่ละลูกแยกตามหมายเลขให้ชัดเจนที่สุด",
...
• **หัวข้อ Battery (แบตเตอรี่ - น้ำกลั่นและสภาพขั้วแบต):**
  - **100 คะแนน:** ระดับน้ำกลั่นทุกลูกอยู่ระหว่างขีด Upper-Lower เห็นชัดเจนในรูป ขั้วแบตไม่มีคราบเกลือ
  - **50 คะแนน:** มีบางลูกระดับน้ำเริ่มต่ำกว่ากึ่งกลางแต่ยังไม่พ้นขีดล่าง หรือภาพถ่ายเบลอจนระบุระดับได้ยาก
  - **0 คะแนน:** น้ำกลั่นแห้งต่ำกว่าขีดล่าง หรือพบความผิดปกติที่ตัวถังแบตเตอรี่ (เช่น บวม แตก ร้าว รั่วซึม หรือคราบซัลเฟตสะสมหนามาก)
...
In the 'summary' field, provide a detailed analysis in Thai about what was surveyed and evaluated referring to the audit criteria.

ตอบกลับในรูปแบบ JSON เท่านั้น โดยมีโครงสร้างดังนี้:
{
  "status": "Green" (เมื่อไม่มีข้อชำรุดบกพร่องตามเกณฑ์คะแนนต่ำ เช่น ประเมินได้คะแนนเต็ม) หรือ "Red" (เมื่อได้คะแนนต่ำ หรือมีสิ่งบกพร่องสำคัญอย่างเห็นได้ชัด),
  "findings": ["Weed", "Bird Droppings", "Low Water Level", "Dirty Terminals", ...], (ระบุคำหลักข้อขัดข้องที่ตรวจพบ),
  "summary": "สรุปรายงานผลการวิเคราะห์อย่างละเอียดเป็นภาษาไทย",
  "category": "Battery" | "Yard" | "Checklist" | "Roof" | "Fence" | "Security" | "Other",
  "score": 100
}`;
