  ...
  useEffect(() => {
    const startCamera = async () => {
      try {
        ...
        setError("ไม่สามารถเปิดกล้องได้โดยตรง กรุณากด 'อนุญาต' (Allow) เพื่อถ่ายภาพสด");
      }
    };
    ...
    >
      <div className="relative flex-1 bg-black flex items-center justify-center overflow-hidden">
        {error ? (
          <div className="text-white text-center p-8 max-w-xs">
            ...
            <h3 className="text-lg font-bold mb-2">การเข้าถึงกล้องถูกปฏิเสธ</h3>
            <p className="text-slate-400 text-sm mb-8 leading-relaxed">
              ระบบจำเป็นต้องใช้กล้องเพื่อถ่ายภาพสดเท่านั้น กรุณาอนุญาตการเข้าถึงกล้องในเบราว์เซอร์ของคุณ
            </p>
            <Button onClick={onClose} variant="outline" className="w-full text-white border-white/20">
              กลับไปลองใหม่
            </Button>
          </div>
        ) : (
          <>
            ...
            <div className="absolute inset-0 pointer-events-none border-[2px] border-white/20 m-10 rounded-3xl" />
          </>
        )}
        
        <button 
          onClick={onClose}
          className="absolute top-6 right-6 w-10 h-10 bg-black/40 text-white rounded-full flex items-center justify-center backdrop-blur-md border border-white/10 z-20"
        >
      ...
      {isReady && !error && (
        <div className="h-32 bg-black flex items-center justify-center relative">
          ...
          <p className="absolute bottom-4 text-[10px] text-white/40 font-bold uppercase tracking-widest">
            LIVE CAMERA MODE
          </p>
        </div>
      )}
      ...
      <div className="max-w-md mx-auto">
        ...
        <div className="space-y-8">
          <div className="p-3 bg-violet-100 border border-violet-200 rounded-xl flex items-center gap-3 text-violet-700 text-[10px] font-bold uppercase tracking-wider">
            ...
            <p>โหมดถ่ายภาพสดเท่านั้น: ระบบบังคับใช้กล้องเพื่อป้องกันการอัปโหลดจากอัลบั้ม</p>
          </div>
